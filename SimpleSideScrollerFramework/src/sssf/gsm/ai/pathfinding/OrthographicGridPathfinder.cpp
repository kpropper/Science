...
void OrthographicGridPathfinder::updatePath(AnimatedSprite *sprite)
{
	...
	if ((currentPath->size() > 0) && (currentNode != currentPath->end()))
	{
		...
		//UPDATES ANIMATION STATE DEPENDING ON DIRECTION OF TRAVEL
		if (vY > 0)
		{
			if (vX > 0)
			{
				sprite->setCurrentState(L"MOVING_TOWARD_RIGHT");
			}
			else if (vX < 0)
			{
				sprite->setCurrentState(L"MOVING_TOWARD_LEFT");
			}
			else
			{
				sprite->setCurrentState(L"MOVING_TOWARD");
			}
		}
		else if (vY < 0)
		{
			if (vX > 0)
			{
				sprite->setCurrentState(L"MOVING_AWAY_RIGHT");
			}
			else if (vX < 0)
			{
				sprite->setCurrentState(L"MOVING_AWAY_LEFT");
			}
			else
			{
				sprite->setCurrentState(L"MOVING_AWAY");
			}
		}
		else
		{
			if (vX > 0)
			{
				sprite->setCurrentState(L"MOVING_RIGHT");
			}

			else
			{
				sprite->setCurrentState(L"MOVING_LEFT");
			}
		}

		//USED FOR TOPDOWN SPRITE WHEN A SINGLE ANIMATION STATE IS USED AND ROTATED
		//COMMENTED OUT AS DIFFERENT ANNIMATION STATED ARE USED BASED ON DIRECTION
